Sub HighlightDuplicates()
    ...
    ' Loop through column F again to highlight rows with names appearing 3 or more times
    For i = 2 To lastRow
        cellValue = Trim(ws.Cells(i, 6).Value) ' Column F - Trim spaces
        If cellValue <> "" Then
            If nameCounts(cellValue) >= 3 Then
                ws.Rows(i).Interior.Color = RGB(255, 0, 0) ' Highlight row in red
            End If
        End If
    Next i
    
    MsgBox "Rows with names appearing 3 or more times have been highlighted in red.", vbInformation
End Sub
