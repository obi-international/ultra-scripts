Sub AddValueToWeightColumn()
    ...
    For i = startRow To lastRow
        ' Read the current weight value and add the user-provided value
        On Error Resume Next
        cellValue = CDbl(ws.Cells(i, weightColumn).Value)
        On Error GoTo 0
        
        If IsNumeric(cellValue) Then
            ws.Cells(i, weightColumn).Value = cellValue + addedValue
        End If
    Next i
    
    MsgBox "Processing complete. The value " & addedValue & " has been added to the weights in column K."
End Sub
